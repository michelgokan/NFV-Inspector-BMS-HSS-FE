...
function Nfvinspectorbmshssfeplugin(Nfvinspectorbmshssfeplugin) {
                            ...
                            if (error === null && response.statusCode == 200) {
                                ...
                                response_lines.forEach(function (line) {
                                    if (regex.test(line)) {
                                        ...
                                        var start_time_object = moment.tz(start_time, "Europe/Stockholm");
                                        var end_time_object = moment.tz(end_time, "Europe/Stockholm");
    ...
    Nfvinspectorbmshssfeplugin.reinstantiateSystem = async function () {
        var configs = getHSSFEConfigs(Nfvinspectorbmshssfeplugin);

        var result = configs.then(function (hss_configs) {
            var exec = require('child_process').exec;
            var command = path.dirname(require.main.filename) +
                "/../Plugins/nfv-inspector-bms-hss-fe-plugin/src/hss-core/reinstantiate.sh";
            ...
            var executionPromise = new Promise(function (resolve, reject) {
                exec(command, {maxBuffer: 1024 * 10000}, function (error, stdout, stderr) {
                    try {
                        console.log("RESOLVE INSTANTIATION");
                        resolve(stdout);
                    } catch (e) {
                        console.log("REJECT INSTANTIATION");
                        resolve(stdout);
                    }
                });
            });
    ...
    Nfvinspectorbmshssfeplugin.remoteMethod('reinstantiateSystem', {
        returns: {arg: 'result', type: 'json'},
        http: {path: '/reinstantiateSystem', verb: 'get'}
    });
